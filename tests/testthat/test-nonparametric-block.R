test_that("nonparametric block produces an htest on independent samples", {
  block <- new_nonparametric_block(
  ...
  shiny::testServer(
    blockr.core:::get_s3_method("block_server", block),
    {
      ...
      expect_true(grepl("Wilcoxon", result$method))
    },
    args = list(x = block, data = list(data = function() .tdf_a()))
  )
})

test_that("nonparametric block clears stale column state on data swap", {
